...
function getPackageInfo() {
    try {
        ...
        throw createError('Failed to read package.json', ERROR_CODES.CONFIG_VALIDATION_ERROR, { error: error instanceof Error ? error.message : String(error) });
    }
}
/**
 ...
 */
function parseEnvironmentVariables(env, projectConfig = {}) {
    const config = {};
    const errors = [];
    ...
    if (hasProjectConfig) {
        ...
        const configuredPath = projectConfig['MCP_WORKSPACE_PATH'] ?? env['MCP_WORKSPACE_PATH'];
    ...
    let workflowsBasePath;
    if (hasProjectConfig) {
        // Project config exists - use project config value or default (ignore process.env)
        workflowsBasePath = projectConfig['WORKFLOWS_BASE_PATH'] ?? defaultWorkflowsPath;
    }
    else {
    ...
    if (hasProjectConfig) {
        // Project config exists - use project config value or default (ignore process.env)
        guidesBasePath = projectConfig['GUIDES_BASE_PATH'] ?? defaultGuidesPath;
    }
    else {
        // No project config - use process.env or default
    ...
    if (workspaceBasePath != null) {
        if (hasProjectConfig) {
            const projectDocsPath = projectConfig['DOCS_BASE_PATH'];
    ...
    let archivedBasePath;
    if (workspaceBasePath != null) {
        if (hasProjectConfig) {
            const projectArchivedPath = projectConfig['ARCHIVED_BASE_PATH'];
    ...
    let coordinatorBasePath;
    if (workspaceBasePath != null) {
        if (hasProjectConfig) {
            const projectCoordinatorPath = projectConfig['COORDINATOR_BASE_PATH'];
    ...
    if (errors.length > 0) {
        throw createError(`Missing required configuration: ${errors.join(', ')}`, ERROR_CODES.ENVIRONMENT_ERROR, { errors, receivedEnv: env });
    }
    return config;
}
/**
 ...
 */
function logConfigurationStartup(config, hasProjectConfig) {
    const logger = getGlobalLogger();
...
export function loadConfig() {
    try {
        ...
        if (!result.success) {
            ...
            throw createError(`Configuration validation failed: ${errors}`, ERROR_CODES.CONFIG_VALIDATION_ERROR, {
        ...
        throw createError('Failed to load configuration', ERROR_CODES.ENVIRONMENT_ERROR, { error: error instanceof Error ? error.message : String(error) });
    }
}
/**
...
export function getWorkspacePath(workspacePath) {
 ...
 */
// ts-unused-exports:disable-next-line
export function getWorkflowsPath(workflowsPath) {
 ...
 */
// ts-unused-exports:disable-next-line
export function getGuidesPath(guidesPath) {
 ...
 */
export function getDocsPath(docsPath) {
 ...
 */
export function getArchivedPath(archivedPath) {
 ...
 */
// ts-unused-exports:disable-next-line
export function getCoordinatorPath(coordinatorPath) {
