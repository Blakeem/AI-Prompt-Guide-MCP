 ...
 */
async function validateAndSanitizePath(filePath, operation) {
    // Input validation
    if (typeof filePath !== 'string') {
        throw createError(`Invalid path type: expected string, got ${typeof filePath}`, ERROR_CODES.INVALID_OPERATION, { operation, pathType: typeof filePath });
    }
    // Check path length to prevent resource exhaustion
    if (filePath.length > MAX_PATH_LENGTH) {
        throw createError(`Path too long: ${filePath.length} characters exceeds maximum of ${MAX_PATH_LENGTH}`, ERROR_CODES.INVALID_OPERATION, { operation, pathLength: filePath.length, maxLength: MAX_PATH_LENGTH });
    }
    ...
    if (sanitizedPath === '') {
        throw createError('Path is empty or contains only dangerous characters', ERROR_CODES.INVALID_OPERATION, { operation, originalPath: filePath });
    }
    ...
    try {
        ...
        if (resolvedPath.startsWith(docsBasePath) === false) {
            ...
            throw createError('Path traversal attempt detected', ERROR_CODES.INVALID_OPERATION, {
                operation,
                attemptedPath: filePath,
                resolvedPath: relative(docsBasePath, resolvedPath),
        ...
        if (extension !== '' && !ALLOWED_EXTENSIONS.has(extension)) {
            ...
            throw createError(`File extension not allowed: ${extension}`, ERROR_CODES.INVALID_OPERATION, {
                operation,
                path: filePath,
                extension,
        ...
        throw createError(`Path validation failed: ${error instanceof Error ? error.message : String(error)}`, ERROR_CODES.INVALID_OPERATION, {
            operation,
            path: filePath,
 ...
 */
function validateFileSize(size, operation, path) {
    if (size > DEFAULT_LIMITS.MAX_FILE_SIZE) {
        throw createError(`File size ${size} bytes exceeds maximum allowed size of ${DEFAULT_LIMITS.MAX_FILE_SIZE} bytes`, ERROR_CODES.FILE_TOO_LARGE, {
            operation,
 ...
 */
function validateContentLength(content, operation, path) {
 ...
 */
async function readFileSnapshotInternal(absolutePath, operation) {
    try {
        ...
        if (error instanceof Error && 'code' in error) {
            if (error.code === 'ENOENT') {
                throw createError(`File not found: ${basename(absolutePath)}`, ERROR_CODES.FILE_NOT_FOUND, {
                    operation,
            ...
            if (error.code === 'EACCES') {
                throw createError(`Access denied: ${basename(absolutePath)}`, ERROR_CODES.INVALID_OPERATION, {
                    operation,
            ...
            if (error.code === 'EISDIR') {
                throw createError(`Path is a directory, not a file: ${basename(absolutePath)}`, ERROR_CODES.INVALID_OPERATION, {
                    operation,
                    path: basename(absolutePath),
 ...
 */
export async function readFileSnapshot(path, options) {
 ...
 * @throws SpecDocsError for file system errors
 */
async function writeFileIfUnchangedInternal(absolutePath, expectedMtimeMs, content, operation) {
    ...
    if (typeof expectedMtimeMs !== 'number' ||
        ...
        throw createError(`Invalid expectedMtimeMs: ${expectedMtimeMs}. Must be a valid timestamp between Unix epoch (0) and 1 year in the future.`, ERROR_CODES.INVALID_OPERATION, {
            operation,
            expectedMtimeMs,
            validRange: {
    ...
    try {
        ...
        if (stat.mtimeMs !== expectedMtimeMs) {
            throw createError('File has been modified by another process', ERROR_CODES.PRECONDITION_FAILED, {
                operation,
                path: basename(absolutePath), // Only include filename for security
                expectedMtimeMs,
        ...
        if (error instanceof Error && 'code' in error) {
            ...
            if (error.code === 'ENOSPC') {
                throw createError('Insufficient disk space', ERROR_CODES.INVALID_OPERATION, {
                    operation,
                    path: basename(absolutePath),
 ...
 * @throws SpecDocsError for security violations or file system errors
 */
export async function writeFileIfUnchanged(path, expectedMtimeMs, content, options) {
...
export async function ensureDirectoryExists(dirPath) {
    ...
    if (typeof dirPath !== 'string' || dirPath.trim() === '') {
        throw createError('Directory path must be a non-empty string', ERROR_CODES.INVALID_OPERATION, { operation, providedPath: dirPath });
    }
    ...
    const sanitizedPath = dirPath
        ...
        .trim();
    if (sanitizedPath === '') {
        throw createError('Directory path contains only dangerous characters', ERROR_CODES.INVALID_OPERATION, { operation, originalPath: dirPath });
    }
    ...
    try {
        ...
        if (error instanceof Error && 'code' in error) {
            ...
            if (error.code === 'EACCES') {
                throw createError(`Access denied creating directory: ${basename(absolutePath)}`, ERROR_CODES.INVALID_OPERATION, {
                    operation,
                    path: basename(absolutePath),
                    securityViolation: 'ACCESS_DENIED'
                });
            ...
            if (error.code === 'ENOSPC') {
                throw createError('Insufficient disk space to create directory', ERROR_CODES.INVALID_OPERATION, {
                    operation,
                    path: basename(absolutePath),
                    securityViolation: 'DISK_FULL'
                });
