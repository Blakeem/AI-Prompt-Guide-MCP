...
`;
    ...
    it('should handle sequential insertSection operations without race condition', async () => {
        ...
        await manager.insertSection(testDocPath, 'section-a', 'insert_after', 2, 'Section C', 'Content C', { updateToc: false });
        // Operation 2: Insert section D after C (depends on C existing)
        // This should succeed if cache is invalidated after first insert
        await expect(manager.insertSection(testDocPath, 'section-c', 'insert_after', 2, 'Section D', 'Content D', { updateToc: false })).resolves.not.toThrow();
        // Operation 3: Insert section E after D (depends on D existing)
        await expect(manager.insertSection(testDocPath, 'section-d', 'insert_after', 2, 'Section E', 'Content E', { updateToc: false })).resolves.not.toThrow();
    ...
    it('should handle sequential updateSection operations without race condition', async () => {
        ...
        await manager.updateSection(testDocPath, 'section-a', 'Updated content A', { updateToc: false });
        // Operation 2: Update section B (should see updated document state)
        await expect(manager.updateSection(testDocPath, 'section-b', 'Updated content B', { updateToc: false })).resolves.not.toThrow();
    ...
    it('should handle sequential renameSection operations without race condition', async () => {
        ...
        await manager.renameSection(testDocPath, 'section-a', 'Renamed Section A', { updateToc: false });
        // Operation 2: Rename section B (should see updated document state)
        await expect(manager.renameSection(testDocPath, 'section-b', 'Renamed Section B', { updateToc: false })).resolves.not.toThrow();
    ...
    it('should handle sequential deleteSection operations without race condition', async () => {
        ...
        await manager.insertSection(testDocPath, 'section-b', 'insert_after', 2, 'Section C', 'Content C', { updateToc: false });
        cache.invalidateDocument(testDocPath);
        ...
        await expect(manager.deleteSection(testDocPath, 'section-b', { updateToc: false })).resolves.not.toThrow();
    ...
    it('should handle mixed batch operations (insert, update, delete) without race condition', async () => {
        ...
        await manager.updateSection(testDocPath, 'section-a', 'Updated A', { updateToc: false });
...
//# sourceMappingURL=document-manager.race-condition.test.js.map