...
import { getWorkflowPrompts, getWorkflowPrompt } from '../../prompts/workflow-prompts.js';
import { createErrorResponse, formatLogError, } from '../../utils/error-formatter.js';
/**
 * Convert workflow prompt to MCP prompt format
 */
function convertToMCPPrompt(workflowPrompt) {
    return {
 ...
 */
export function registerPromptHandlers(server, _serverConfig) {
    ...
    server.setRequestHandler(GetPromptRequestSchema, async (request) => {
        ...
        try {
            ...
            if (workflowPrompt == null) {
                ...
                return createErrorResponse(`Prompt '${name}' not found. Use prompts/list to see available prompts.`);
            }
            ...
            return createErrorResponse(`Failed to retrieve prompt '${name}': ${error instanceof Error ? error.message : String(error)}`);
        }
    });
}
