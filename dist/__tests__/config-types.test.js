...
describe('ProjectConfig Types and Schema', () => {
    ...
    describe('Invalid configurations', () => {
        ...
        it('should reject config with MCP_WORKSPACE_PATH as non-string (number)', () => {
            ...
            expect(result.success).toBe(false);
            if (!result.success) {
                const docsPathError = result.error.issues.find(issue => issue.path.includes('MCP_WORKSPACE_PATH'));
        ...
        it('should reject config with WORKFLOWS_BASE_PATH as non-string (boolean)', () => {
            ...
            expect(result.success).toBe(false);
            if (!result.success) {
                const workflowsPathError = result.error.issues.find(issue => issue.path.includes('WORKFLOWS_BASE_PATH'));
        ...
        it('should reject config with GUIDES_BASE_PATH as non-string (array)', () => {
            ...
            expect(result.success).toBe(false);
            if (!result.success) {
                const guidesPathError = result.error.issues.find(issue => issue.path.includes('GUIDES_BASE_PATH'));
...
//# sourceMappingURL=config-types.test.js.map