...
describe('Config Path Validation', () => {
    ...
    describe('Optional WORKFLOWS_BASE_PATH Validation', () => {
        it('should log warning when WORKFLOWS_BASE_PATH does not exist', () => {
            ...
            expect(loggerSpy.warn).toHaveBeenCalledWith(expect.stringContaining('WORKFLOWS_BASE_PATH directory does not exist'), expect.objectContaining({
        ...
        it('should not log warning when WORKFLOWS_BASE_PATH exists', () => {
            ...
            expect(loggerSpy.warn).not.toHaveBeenCalledWith(expect.stringContaining('WORKFLOWS_BASE_PATH directory does not exist'), expect.any(Object));
        });
    });
    describe('Optional GUIDES_BASE_PATH Validation', () => {
        it('should log warning when GUIDES_BASE_PATH does not exist', () => {
            ...
            expect(() => loadConfig()).not.toThrow();
            // Assert: Warning should be logged
            expect(loggerSpy.warn).toHaveBeenCalledWith(expect.stringContaining('GUIDES_BASE_PATH directory does not exist'), expect.objectContaining({
                path: expect.any(String)
            }));
        });
        it('should not log warning when GUIDES_BASE_PATH exists', () => {
            ...
            expect(loggerSpy.warn).not.toHaveBeenCalledWith(expect.stringContaining('GUIDES_BASE_PATH directory does not exist'), expect.any(Object));
        });
    });
    describe('Multiple Path Validation', () => {
        it('should log warnings for multiple non-existent optional paths', () => {
            ...
            expect(loggerSpy.warn).toHaveBeenCalledWith(expect.stringContaining('WORKFLOWS_BASE_PATH directory does not exist'), expect.any(Object));
            expect(loggerSpy.warn).toHaveBeenCalledWith(expect.stringContaining('GUIDES_BASE_PATH directory does not exist'), expect.any(Object));
        });
        it('should not log warnings when all paths exist', () => {
            ...
            expect(loggerSpy.warn).not.toHaveBeenCalledWith(expect.stringMatching(/BASE_PATH directory does not exist/), expect.any(Object));
        });
    });
    describe('Project Config Path Validation', () => {
        it('should validate paths from .mcp-config.json', () => {
            ...
            writeFileSync(join(tempDir, '.mcp-config.json'), JSON.stringify(configContent, null, 2));
...
//# sourceMappingURL=config-path-validation.test.js.map