 ...
 */
export async function continueTask(args, _state, manager) {
    try {
        // ===== INPUT VALIDATION =====
        ...
        if (addresses.task == null) {
            throw new AddressingError('Task address is required', 'MISSING_TASK_ADDRESS');
        }
        // ===== DOCUMENT LOADING =====
        ...
        if (taskContent == null) {
            throw new AddressingError(`Task content not found for ${taskAddress.slug}`, 'TASK_CONTENT_NOT_FOUND', {
                document: addresses.document.path,
                task: taskAddress.slug
            });
        }
        ...
        const enrichedTask = await enrichTaskWithReferences(manager, addresses.document.path, taskAddress.slug, taskContent, heading, taskAddress);
        // Merge workflow data with reference-enriched data
        // Extract workflow data with explicit typing
        const workflow = 'workflow' in taskWithBothWorkflows
            ? taskWithBothWorkflows.workflow
            : undefined;
        const mainWorkflow = 'mainWorkflow' in taskWithBothWorkflows
            ? taskWithBothWorkflows.mainWorkflow
            : undefined;
        const fullyEnriched = {
            ...enrichedTask,
        ...
        throw new AddressingError(`Failed to continue task: ${error instanceof Error ? error.message : String(error)}`, 'CONTINUE_TASK_FAILED', {
