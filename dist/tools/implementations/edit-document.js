...
export async function editDocument(args, _state, manager) {
    try {
        // Input validation
        ...
        if (newTitle == null && newOverview == null) {
            throw new AddressingError('At least one of title or overview must be provided', 'MISSING_PARAMETERS', { document: documentPath });
        }
        ...
        if (newTitle != null && newTitle !== '') {
            ...
            if (titleHeading != null) {
                ...
                throw new AddressingError('Document has no title (H1 heading) to update', 'NO_TITLE_FOUND', { document: addresses.document.path });
            }
        }
        ...
        if (newOverview != null && newOverview !== '') {
            ...
            if (titleLineIndex === -1) {
                throw new AddressingError('Document has no title (H1 heading)', 'NO_TITLE_FOUND', { document: addresses.document.path });
            }
        ...
        throw new AddressingError(`Failed to edit document: ${error instanceof Error ? error.message : String(error)}`, 'EDIT_DOCUMENT_FAILED', { document: args['document'] });
    }
}
