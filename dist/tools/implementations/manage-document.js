...
export async function manageDocument(args, _state, manager) {
    try {
        ...
        throw new AddressingError(`Failed to manage document: ${message}`, 'DOCUMENT_MANAGE_ERROR', { args, originalError: message });
    }
}
/**
 * Perform a single document management operation
 */
async function performDocumentOperation(manager, operation, docPath, options) {
    ...
    const createDocumentInfo = (docAddress, doc) => {
    ...
    const validatedOperation = ToolIntegration.validateOperation(operation, ['archive', 'delete', 'rename'], 'manage-document');
