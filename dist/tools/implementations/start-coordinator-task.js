 ...
 */
export async function startCoordinatorTask(args, _state, manager) {
    try {
        // Extract return_task_context parameter (default: false)
        const returnTaskContext = args['return_task_context'] ?? false;
        ...
        if (nextTask == null) {
            throw new AddressingError('No available tasks in coordinator document', 'NO_AVAILABLE_TASKS', {
        ...
        const withReferences = await enrichTaskWithReferences(manager, documentPath, targetTaskSlug, taskContent);
        ...
        const workflow = 'workflow' in fullyEnriched
            ? fullyEnriched.workflow
            : undefined;
        const mainWorkflow = 'mainWorkflow' in fullyEnriched
            ? fullyEnriched.mainWorkflow
            : undefined;
        ...
        throw new AddressingError(`Failed to start coordinator task: ${error instanceof Error ? error.message : String(error)}`, 'START_COORDINATOR_TASK_FAILED', {
            originalError: error instanceof Error ? error.message : String(error)
        });
    }
}
//# sourceMappingURL=start-coordinator-task.js.map