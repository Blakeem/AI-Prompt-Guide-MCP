...
import { PATH_PREFIXES, ARCHIVE_PREFIXES, FOLDER_NAMES } from '../../shared/namespace-constants.js';
/**
 ...
 */
export async function completeCoordinatorTask(args, _state, manager) {
    try {
        ...
        if (currentTask == null) {
            throw new AddressingError('No available tasks to complete', 'NO_AVAILABLE_TASKS', { document: documentPath });
        }
        // Complete the task using shared operation
        const completedTaskData = await completeTaskOperation(manager, documentPath, currentTask.slug, note);
        ...
        if (returnNextTask) {
            ...
            if (nextTaskData != null) {
                ...
                const enrichedNext = enrichTaskWithWorkflow({ slug: nextTaskData.slug, title: nextTaskData.title, content: nextTaskContent, status: nextTaskData.status }, nextTaskContent);
                ...
                if ('workflow' in enrichedNext && enrichedNext.workflow != null) {
                    const fullWorkflow = enrichedNext.workflow;
        ...
        throw new AddressingError(`Failed to complete coordinator task: ${error instanceof Error ? error.message : String(error)}`, 'COMPLETE_COORDINATOR_TASK_FAILED', {
 ...
 */
async function checkAllTasksComplete(manager, document) {
 ...
 */
async function archiveCoordinatorDocument(manager, documentPath) {
...
//# sourceMappingURL=complete-coordinator-task.js.map