 ...
 */
export async function startTask(args, _state, manager) {
    try {
        ...
        if (mode === 'adhoc') {
            ...
            if (nextTask == null) {
                ...
                if (taskHeadings.length === 0) {
                    throw new AddressingError('No tasks found in document', 'NO_TASKS_FOUND', {
                        document: addresses.document.path
                    });
                }
        ...
        const enrichedTask = await enrichTaskWithReferences(manager, addresses.document.path, targetTaskSlug, taskContent, heading, taskAddressForEnrichment);
