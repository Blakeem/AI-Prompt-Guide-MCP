 ...
 */
export async function executeGetWorkflow(args, _state, _manager) {
    // IPO Pattern: Input Collection
    const workflow = args['workflow'];
    // Input Validation
    if (workflow == null || workflow === '') {
        throw new AddressingError('workflow parameter is required', 'INVALID_PARAMETER');
    }
    if (typeof workflow !== 'string') {
        throw new AddressingError('workflow parameter must be a string', 'INVALID_PARAMETER');
    }
