...
import { ToolIntegration, AddressingError, DocumentNotFoundError, SectionNotFoundError, } from '../../shared/addressing-system.js';
import { isValidMovePosition } from '../schemas/move-schemas.js';
 ...
 */
export async function move(args, _state, manager) {
    ...
    if (!isValidMovePosition(positionRaw)) {
        throw new AddressingError(`Invalid position: ${positionRaw}. Must be one of: before, after, child`, 'INVALID_POSITION', { position: positionRaw });
    }
    const position = positionRaw;
    // Parse source path - must include section slug
    if (!fromPath.includes('#')) {
        throw new AddressingError('Source path must include section slug (e.g., "/docs/api/auth.md#section")', 'INVALID_SOURCE_PATH', { from: fromPath });
    }
    const [sourceDocPath, sourceSection] = fromPath.split('#');
    if (sourceDocPath == null || sourceSection == null || sourceSection === '') {
        throw new AddressingError('Invalid source path format. Expected: "/document.md#section"', 'INVALID_SOURCE_FORMAT', { from: fromPath });
    }
    ...
    const sourceHeading = sourceDoc.headings.find((h) => h.slug === sourceSectionAddress.slug);
    if (sourceHeading == null) {
        throw new SectionNotFoundError(sourceSectionAddress.slug, sourceAddresses.document.path);
    }
    // Get source content (what we'll move)
    const sourceContent = await manager.getSectionContent(sourceAddresses.document.path, sourceSectionAddress.slug);
    if (sourceContent == null) {
        throw new AddressingError('Failed to read source section content', 'SOURCE_CONTENT_ERROR', {
    ...
    if (refHeading == null) {
        throw new SectionNotFoundError(normalizedReference, destAddresses.document.path);
    }
    // Map position to InsertMode for performSectionEdit
    const insertMode = position === 'before'
        ? 'insert_before'
        : position === 'after'
            ? 'insert_after'
            : 'append_child';
    ...
    try {
        if (isSameDocument) {
            ...
            try {
                ...
                await performSectionEdit(manager, destAddresses.document.path, normalizedReference, contentBackup, insertMode, titleBackup);
            }
            catch (createError) {
                // Rollback: Restore section at original location
                // This is best-effort - if restore fails, content may be lost
                try {
                    await performSectionEdit(manager, sourceAddresses.document.path, normalizedReference, // Use reference as anchor for restoration
                    contentBackup, 'insert_before', // Use insert_before as safe default
                    titleBackup);
                }
                catch (rollbackError) {
                    // Rollback failed - content is lost
                    const errorMsg = rollbackError instanceof Error ? rollbackError.message : String(rollbackError);
                    throw new AddressingError(`Same-document move failed and rollback failed. Content may be lost. Original error: ${createError instanceof Error ? createError.message : String(createError)}. Rollback error: ${errorMsg}`, 'MOVE_ROLLBACK_FAILED', {
                        from: fromPath,
                        to: toPath,
            ...
            await performSectionEdit(manager, destAddresses.document.path, normalizedReference, sourceContent, insertMode, sourceTitle);
        ...
        if (error instanceof Error) {
            throw new AddressingError(`Move operation failed: ${error.message}. Note: If creation succeeded but deletion failed, content may be duplicated.`, 'MOVE_OPERATION_FAILED', {
                from: fromPath,
                to: toPath,
                reference: normalizedReference,
                position,
