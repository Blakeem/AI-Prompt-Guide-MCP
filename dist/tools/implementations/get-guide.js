 ...
 */
export async function executeGetGuide(args, _state, _manager) {
    // IPO Pattern: Input Collection
    const guide = args['guide'];
    // Input Validation
    if (guide == null || guide === '') {
        throw new AddressingError('guide parameter is required', 'INVALID_PARAMETER');
    }
    if (typeof guide !== 'string') {
        throw new AddressingError('guide parameter must be a string', 'INVALID_PARAMETER');
    }
