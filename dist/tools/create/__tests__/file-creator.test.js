...
describe('createDocumentFile', () => {
    ...
    describe('Tasks section creation', () => {
        it('should create document without Tasks section', async () => {
            ...
            const result = await createDocumentFile('test-file-creator', 'Test Document', 'Test overview', manager, content, docPath, 'test-document');
            expect(result).toHaveProperty('success', true);
            if ('success' in result && result.success) {
                const document = await manager.getDocument(docPath);
                expect(document).toBeDefined();
                // Should not have Tasks section
                const tasksHeading = document?.headings.find(h => h.title.toLowerCase() === 'tasks');
    ...
    describe('Document creation validation', () => {
        it('should reject invalid document path', async () => {
            ...
            const result = await createDocumentFile('test-file-creator', 'Test', 'Content', manager, content, invalidPath, 'test');
        ...
        it('should handle empty content gracefully', async () => {
            ...
            const result = await createDocumentFile('test-file-creator', 'Empty Document', 'No content', manager, content, docPath, 'empty-document');
    ...
    describe('Cache refresh behavior', () => {
        it('should refresh cache after document creation', async () => {
            ...
            const result = await createDocumentFile('test-file-creator', 'Fresh Document', 'Content here', manager, content, docPath, 'fresh-document');
            expect(result).toHaveProperty('success', true);
...
//# sourceMappingURL=file-creator.test.js.map