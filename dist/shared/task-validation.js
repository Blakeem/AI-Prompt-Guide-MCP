 ...
 */
export function validateSubagentTaskAccess(documentPath, taskSlug) {
    ...
    if (!isDocsPath(documentPath)) {
        throw new AddressingError(`Subagent tools only work in ${PATH_PREFIXES.DOCS} namespace. Use coordinator tools for /coordinator/ tasks.`, 'INVALID_NAMESPACE_FOR_SUBAGENT', { documentPath, expected_namespace: PATH_PREFIXES.DOCS });
    }
}
/**
 ...
 * @throws {AddressingError} When validation fails
 */
export function validateCoordinatorTaskAccess(documentPath, taskSlug) {
    // Rule 1: Must NOT have task slug (sequential only)
    if (taskSlug != null && taskSlug !== '') {
        throw new AddressingError('Coordinator tasks are sequential only. Do not specify #slug.', 'COORDINATOR_NO_SLUG_ALLOWED', {
    ...
    if (!isCoordinatorPath(documentPath)) {
        throw new AddressingError(`Coordinator tools only work with ${PATH_PREFIXES.COORDINATOR} namespace`, 'INVALID_COORDINATOR_PATH', {
            documentPath,
 ...
 */
export function validateMainWorkflowPresence(isFirstTask, hasMainWorkflow, taskTitle) {
