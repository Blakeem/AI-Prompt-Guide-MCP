...
import { extractWeightedKeywords, extractKeywordsWithFingerprint, calculateWeightedRelevance, calculateWeightedRelevanceWithBreakdown, generateFactorBasedExplanation, extractKeywords, calculateNamespaceAffinity, calculateRecencyBoost, calculateTitleSimilarity, calculateLinkGraphBoost, DEFAULT_KEYWORD_WEIGHTS, DEFAULT_SCORING_FEATURES } from '../keyword-utils.js';
describe('Enhanced Keyword Utilities', () => {
    describe('extractWeightedKeywords', () => {
        it('should extract keywords from title and content', () => {
            const result = extractWeightedKeywords('User Authentication API', 'This document describes JWT authentication for user management');
...
Regular content with keywords.`;
    ...
    describe('extractKeywordsWithFingerprint', () => {
        it('should prefer content analysis when content is available', () => {
            ...
            const result = extractKeywordsWithFingerprint('New Title', 'New content with different keywords', fingerprint);
        ...
        it('should fall back to fingerprint when no content available', () => {
            ...
            const result = extractKeywordsWithFingerprint('Title', null, fingerprint);
        ...
        it('should handle missing fingerprint gracefully', () => {
            const result = extractKeywordsWithFingerprint('Test Title', null, undefined);
    ...
    describe('calculateWeightedRelevance (legacy compatibility)', () => {
        it('should calculate relevance using multi-factor scoring', () => {
            ...
            const relevance = calculateWeightedRelevance(sourceKeywords, 'Source API Documentation', // sourceTitle
        ...
        it('should return 0 for no keyword matches', () => {
            ...
            const relevance = calculateWeightedRelevance(sourceKeywords, 'Database Guide', // sourceTitle
        ...
        it('should handle empty source keywords', () => {
            // Use an old date to avoid recency boost
            const oldDate = new Date('2020-01-01');
            const relevance = calculateWeightedRelevance([], 'Any Title', // sourceTitle
    ...
    describe('extractKeywords (backward compatibility)', () => {
        it('should maintain backward compatibility with original behavior', () => {
            const keywords = extractKeywords('User Authentication API', 'This API handles user login and session management');
...
# Content`;
    ...
    describe('performance and edge cases', () => {
        ...
        it('should handle unicode and special characters', () => {
            const result = extractWeightedKeywords('Café API Documentation', 'Content with émojis 🚀 and spéciàl characters');
            expect(result.keywords.length).toBeGreaterThan(0);
    ...
    describe('Multi-factor Relevance Scoring', () => {
        describe('calculateNamespaceAffinity', () => {
            ...
            it('should handle edge cases gracefully', () => {
                ...
                expect(calculateNamespaceAffinity(null, 'api')).toBe(0.0);
            });
        ...
        describe('calculateRecencyBoost', () => {
            ...
            it('should handle invalid dates gracefully', () => {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                expect(calculateRecencyBoost(null)).toBe(0.0);
            });
        });
        describe('calculateTitleSimilarity', () => {
            ...
            it('should handle edge cases gracefully', () => {
                ...
                expect(calculateTitleSimilarity('API', '')).toBe(0);
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                expect(calculateTitleSimilarity(null, 'API')).toBe(0);
            });
            it('should filter out common stop words', () => {
                // These titles share the word 'quick' which should be meaningful
                const similarity = calculateTitleSimilarity('The Quick Guide', 'A Quick Reference');
                expect(similarity).toBeGreaterThan(0); // 'quick' is meaningful
                // Test with only stop words - should be 0
                const stopWordsOnly = calculateTitleSimilarity('The Guide', 'A Reference');
                expect(stopWordsOnly).toBe(0); // Only stop words, no meaningful overlap
            });
        });
        describe('calculateLinkGraphBoost', () => {
            const features = { enableLinkGraphBoost: true };
            const disabledFeatures = { enableLinkGraphBoost: false };
            it('should return 0.0 when feature is disabled', () => {
                const boost = calculateLinkGraphBoost('See @/api/auth.md for details', '/api/auth.md', disabledFeatures);
                expect(boost).toBe(0.0);
            });
            it('should return 0.3 when reference is found and feature enabled', () => {
                const boost = calculateLinkGraphBoost('See @/api/auth.md for details', '/api/auth.md', features);
                expect(boost).toBe(0.3);
            });
            it('should return 0.0 when no reference is found', () => {
                const boost = calculateLinkGraphBoost('This content has no references', '/api/auth.md', features);
                expect(boost).toBe(0.0);
            });
            it('should use default features when not provided', () => {
                // Default features have linkGraphBoost disabled
                const boost = calculateLinkGraphBoost('See @/api/auth.md for details', '/api/auth.md');
                expect(boost).toBe(0.0);
            });
            it('should handle edge cases gracefully', () => {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                expect(calculateLinkGraphBoost(null, '/api/auth.md', features)).toBe(0.0);
            });
        });
        describe('Multi-factor calculateWeightedRelevance', () => {
            ...
            it('should combine all relevance factors', () => {
                const relevance = calculateWeightedRelevance(sourceKeywords, baseParams.sourceTitle, baseParams.sourceNamespace, baseParams.sourceContent, baseParams.targetTitle, baseParams.targetContent, baseParams.targetNamespace, baseParams.targetLastModified, baseParams.targetPath, undefined, { enableLinkGraphBoost: true });
                expect(relevance).toBeGreaterThan(0);
            ...
            it('should cap total relevance at 1.0', () => {
                ...
                const highScoringKeywords = [
                    { keyword: 'authentication', weight: 10.0, sources: ['title'] },
                    { keyword: 'api', weight: 10.0, sources: ['frontmatter'] }
                ];
                const relevance = calculateWeightedRelevance(highScoringKeywords, 'Authentication API', // Same as target title for max title similarity
                'api/specs', // Same namespace for max affinity
                'Reference to @/api/specs/auth.md', 'Authentication API', 'authentication api implementation', 'api/specs', new Date(), // Recent for max recency boost
                '/api/specs/auth.md', undefined, { enableLinkGraphBoost: true });
                expect(relevance).toBe(1.0);
            });
            it('should handle missing optional parameters', () => {
                const relevance = calculateWeightedRelevance(sourceKeywords, baseParams.sourceTitle, baseParams.sourceNamespace, baseParams.sourceContent, baseParams.targetTitle, baseParams.targetContent, baseParams.targetNamespace, baseParams.targetLastModified, baseParams.targetPath
            ...
            it('should return 0 for empty source keywords', () => {
                const relevance = calculateWeightedRelevance([], baseParams.sourceTitle, baseParams.sourceNamespace, baseParams.sourceContent, baseParams.targetTitle, baseParams.targetContent, baseParams.targetNamespace, baseParams.targetLastModified, baseParams.targetPath);
                expect(relevance).toBe(0);
            });
            it('should provide debug information', () => {
                ...
                expect(() => {
                    calculateWeightedRelevance(sourceKeywords, baseParams.sourceTitle, baseParams.sourceNamespace, baseParams.sourceContent, baseParams.targetTitle, baseParams.targetContent, baseParams.targetNamespace, baseParams.targetLastModified, baseParams.targetPath);
                }).not.toThrow();
            });
            it('should demonstrate practical scoring example from improvement plan', () => {
                ...
                const relevanceA = calculateWeightedRelevance(sourceKeywords, 'User Authentication Guide', // sourceTitle
                'api/guides', // sourceNamespace
                ...
                undefined, DEFAULT_SCORING_FEATURES);
                ...
                const relevanceB = calculateWeightedRelevance(sourceKeywords, 'User Authentication Guide', 'api/guides', 'Reference @/security/jwt-implementation.md', 'JWT Implementation', 'jwt implementation details', 'security', // Different namespace
                new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), // last week
                '/security/jwt-implementation.md', undefined, { enableLinkGraphBoost: true } // Enable link boost for this test
                );
    ...
    describe('factor-based explanation generation', () => {
        describe('generateFactorBasedExplanation', () => {
            ...
            it('should generate explanations for two factors', () => {
                const primaryFactors = [
                    { factor: 'keywordOverlap', score: 0.7, description: 'good keyword overlap' },
            ...
            it('should generate explanations for three factors', () => {
                const primaryFactors = [
                    { factor: 'keywordOverlap', score: 0.6, description: 'shared keywords' },
                    { factor: 'titleSimilarity', score: 0.15, description: 'similar titles' },
                    { factor: 'recencyBoost', score: 0.08, description: 'recently updated' }
                ];
            ...
            it('should handle explicit keywords context', () => {
                const primaryFactors = [
                    { factor: 'keywordOverlap', score: 0.8, description: 'strong keyword overlap' },
                    { factor: 'namespaceAffinity', score: 0.15, description: 'related namespace structure' }
                ];
            ...
            it('should handle namespace context already mentioned', () => {
                const primaryFactors = [
                    { factor: 'namespaceAffinity', score: 0.2, description: 'same namespace' },
                    { factor: 'recencyBoost', score: 0.05, description: 'recent updates' }
                ];
            ...
            it('should handle cross-referenced documentation', () => {
                const primaryFactors = [
                    { factor: 'linkGraphBoost', score: 0.3, description: 'cross-referenced documentation' },
                    { factor: 'keywordOverlap', score: 0.4, description: 'shared keywords' }
                ];
        ...
        describe('calculateWeightedRelevanceWithBreakdown', () => {
            it('should return detailed factor breakdown', () => {
                ...
                const result = calculateWeightedRelevanceWithBreakdown(sourceKeywords, 'User Authentication API', 'api/guides', 'This covers authentication', 'Authentication API Reference', 'API for authentication and user management', 'api/specs', new Date(), '/api/specs/auth.md');
            ...
            it('should handle empty keywords gracefully', () => {
                const result = calculateWeightedRelevanceWithBreakdown([], 'Test Title', 'test/namespace', 'test content', 'Target Title', 'target content', 'target/namespace', new Date(), '/target/path.md');
            ...
            it('should identify same namespace factor', () => {
                const sourceKeywords = [
                    { keyword: 'test', weight: 1.0, sources: ['content'] }
                ];
                const result = calculateWeightedRelevanceWithBreakdown(sourceKeywords, 'Test Document', 'api/specs', 'test content', 'Another Test', 'more test content', 'api/specs', // Same namespace
                new Date(), '/api/specs/another.md');
            ...
            it('should identify related namespace factor', () => {
                ...
                const result = calculateWeightedRelevanceWithBreakdown(sourceKeywords, 'Test Document', 'api/specs', 'test content', 'Another Test', 'more test content', 'api/guides', // Related namespace (same parent)
                new Date(), '/api/guides/another.md');
            ...
            it('should identify title similarity factor', () => {
                const sourceKeywords = [
                    { keyword: 'authentication', weight: 2.0, sources: ['title'] }
                ];
                const result = calculateWeightedRelevanceWithBreakdown(sourceKeywords, 'User Authentication Guide', 'guides', 'authentication content', 'Authentication API Reference', 'auth api content', 'api/specs', new Date(), '/api/specs/auth.md');
                expect(result.factors.titleSimilarity).toBeGreaterThan(0.1);
            ...
            it('should identify recency boost factor', () => {
                ...
                const result = calculateWeightedRelevanceWithBreakdown(sourceKeywords, 'Test Document', 'test', 'test content', 'Target Document', 'test target content', 'test', recentDate, '/test/target.md');
        ...
        describe('explanation examples from improvement plan', () => {
            it('should generate explanation matching improvement plan examples', () => {
                ...
                const primaryFactors = [
                    { factor: 'keywordOverlap', score: 0.5, description: 'good keyword overlap' },
                    { factor: 'namespaceAffinity', score: 0.2, description: 'related namespace structure' }
                ];
                const explanation = generateFactorBasedExplanation(primaryFactors, 'api/specs');
            ...
            it('should handle cross-references with shared concepts', () => {
                ...
                const primaryFactors = [
                    { factor: 'linkGraphBoost', score: 0.3, description: 'cross-referenced documentation' },
                    { factor: 'keywordOverlap', score: 0.25, description: 'shared keywords' }
                ];
            ...
            it('should handle explicit frontmatter keyword matches', () => {
                ...
                const primaryFactors = [
                    { factor: 'keywordOverlap', score: 0.8, description: 'strong keyword overlap' },
                    { factor: 'titleSimilarity', score: 0.2, description: 'very similar titles' }
                ];
...
//# sourceMappingURL=keyword-utils.test.js.map