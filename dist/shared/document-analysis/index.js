 ...
 * ```
 */
export async function analyzeDocumentSuggestions(manager, namespace, title, overview, excludePath) {
    // Input validation
    ...
    if (validationErrors.length > 0) {
        ...
        throw new DocumentAnalysisError(`Input validation failed: ${errorMessages.join('; ')}`, 'analyzeDocumentSuggestions', undefined, [
    ...
    try {
        ...
        throw new DocumentAnalysisError(`Document analysis failed: ${error}${errors.length > 0 ? `. Additional errors: ${errors.join('; ')}` : ''}`, 'analyzeDocumentSuggestions', partialResults, [
 ...
 */
function validateAnalysisInputs(manager, namespace, title, overview) {
    const errors = [];
    if (manager == null) {
        errors.push({
            parameter: 'manager',
