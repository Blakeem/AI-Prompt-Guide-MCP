...
import { extractWeightedKeywords, calculateWeightedRelevanceWithBreakdown, generateFactorBasedExplanation, DEFAULT_SCORING_FEATURES } from './keyword-utils.js';
 ...
 */
export async function findRelatedDocuments(manager, _namespace, title, overview, excludePath) {
    // Input validation
    if (manager == null) {
        throw new DocumentAnalysisError('Document manager is required', 'findRelatedDocuments', [], ['Ensure document manager is properly initialized']);
    }
    if (title == null || title.trim() === '') {
        throw new DocumentAnalysisError('Valid title is required', 'findRelatedDocuments', [], ['Provide a non-empty title string']);
    }
    ...
    try {
        ...
        try {
            ...
            if (typeof manager.listDocumentFingerprints === 'function') {
                ...
                if (fingerprints.length > 0) {
                    ...
                    const pathToFingerprint = new Map();
                    ...
                    for (const docInfo of allDocuments) {
                        try {
                            ...
                            if (document != null) {
                                const fingerprint = fingerprints.find(fp => fp.namespace === document.metadata.namespace &&
        ...
        if (usedFingerprintFiltering) {
            ...
            const fullAnalysis = await Promise.all(candidates.map(async (candidate) => {
                try {
                    ...
                    if (keywordResult.weightedKeywords.length > 0) {
                        ...
                        relevanceResult = calculateWeightedRelevanceWithBreakdown(keywordResult.weightedKeywords, title, // sourceTitle
                    ...
                    if (relevanceResult != null && relevanceResult.primaryFactors.length > 0) {
                        ...
                        reason = generateFactorBasedExplanation(relevanceResult.primaryFactors, docNamespace, keywordResult.hasExplicitKeywords);
                    }
            ...
            const validResults = fullAnalysis
                .filter((result) => result != null && result.relevance > 0.2)
            ...
            const fallbackResults = await Promise.all(candidates.map(async (candidate) => {
                try {
                    const document = await manager.getDocument(candidate.path);
                    if (document == null) {
                        return null;
        ...
        throw new DocumentAnalysisError(`Failed to find related documents: ${error}`, 'findRelatedDocuments', [], [
 ...
 * ```
 */
function calculateContentRelevance(keywords, title, content) {
    // Input validation
    if (!Array.isArray(keywords)) {
        throw new DocumentAnalysisError('Keywords must be an array', 'calculateContentRelevance', undefined, ['Ensure keywords parameter is an array of strings']);
    }
    ...
    try {
        ...
        throw new DocumentAnalysisError(`Content relevance calculation failed: ${error}`, 'calculateContentRelevance', 0, [
 ...
 * ```
 */
function calculateFingerprintRelevance(sourceKeywords, fingerprintKeywords) {
    ...
    try {
        ...
        const normalizedFingerprint = new Set(fingerprintKeywords
            .filter(keyword => typeof keyword === 'string' && keyword.trim().length > 0)
            .map(keyword => keyword.toLowerCase().trim()));
...
//# sourceMappingURL=related-docs.js.map