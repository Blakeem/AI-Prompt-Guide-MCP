...
const createMockDocumentManager = () => {
    ...
    const mockSectionContents = new Map();
    // Section content for /api/auth.md
    const authSections = new Map();
    ...
    return {
        ...
        async getSectionContent(path, slug) {
        ...
        cache: {
            ...
            async readDocumentContent(path) {
...
describe('ReferenceLoader', () => {
    ...
    describe('loadReferences', () => {
        ...
        it('should handle missing sections gracefully', async () => {
            ...
            expect(warnMock).toHaveBeenCalledWith(expect.stringContaining('Section "nonexistent" not found in'));
        ...
        it('should handle missing documents gracefully', async () => {
            ...
            const hierarchy = await loader.loadReferences(refs, mockManager, 1, 0);
            expect(hierarchy).toHaveLength(0);
            expect(warnMock).toHaveBeenCalledWith(expect.stringContaining('Document not found: /nonexistent/doc.md'));
            console.warn = originalWarn;
        });
        ...
        it('should detect and prevent circular references', async () => {
            ...
            expect(warnMock).toHaveBeenCalledWith(expect.stringContaining('Cycle detected for path:'));
            console.warn = originalWarn;
        });
        ...
        it('should validate input parameters', async () => {
            const refs = [];
            await expect(loader.loadReferences(refs, mockManager, -1, 0)).rejects.toThrow('maxDepth must be a non-negative number');
            await expect(loader.loadReferences(refs, mockManager, 3, -1)).rejects.toThrow('currentDepth must be a non-negative number');
        });
        it('should handle non-array input gracefully', async () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const hierarchy = await loader.loadReferences(null, mockManager, 3, 0);
            expect(hierarchy).toEqual([]);
        });
    ...
    describe('loadReferencesFromContent', () => {
        it('should extract and load references from content', async () => {
            ...
            const hierarchy = await loader.loadReferencesFromContent(content, contextPath, mockManager, 2);
        ...
        it('should handle content with no references', async () => {
            ...
            const contextPath = '/current/doc.md';
            const hierarchy = await loader.loadReferencesFromContent(content, contextPath, mockManager);
            expect(hierarchy).toEqual([]);
        });
    ...
    describe('error handling and edge cases', () => {
        it('should handle corrupted document manager responses', async () => {
            ...
            expect(hierarchy).toEqual([]);
            expect(warnMock).toHaveBeenCalledWith(expect.stringContaining('Failed to load reference'), expect.objectContaining({ error: expect.any(String) }));
...
//# sourceMappingURL=reference-loader.test.js.map