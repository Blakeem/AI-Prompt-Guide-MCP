...
describe('ReferenceExtractor', () => {
    ...
    describe('extractReferences', () => {
      ...
      `;
        ...
        it('should handle null/undefined content safely', () => {
            ...
            expect(extractor.extractReferences(null)).toEqual([]);
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            expect(extractor.extractReferences(undefined)).toEqual([]);
        });
    ...
    describe('normalizeReferences', () => {
        ...
        it('should handle non-array input gracefully', () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const normalized = extractor.normalizeReferences(null, contextPath);
            expect(normalized).toEqual([]);
        });
        it('should skip invalid reference strings', () => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const refs = ['@/api/auth.md', '', null, undefined, '@/valid/path.md'];
            const normalized = extractor.normalizeReferences(refs, contextPath);
        ...
        it('should handle references that are not @ prefixed', () => {
            ...
            expect(warnMock).toHaveBeenCalledWith(expect.stringContaining('Failed to normalize reference'), expect.any(Error));
...
//# sourceMappingURL=reference-extractor.test.js.map