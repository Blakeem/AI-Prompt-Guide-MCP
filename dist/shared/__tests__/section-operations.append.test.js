...
`;
        ...
        const result = await performSectionEdit(manager, testDocPath, 'tasks', // Reference section (will insert before this)
        'New section content', 'append', 'New Section');
...
`;
        ...
        const result = await performSectionEdit(manager, testDocPath, 'features', // Will use as reference point
        'New section content', 'append', 'New Section');
        expect(result.action).toBe('created');
        expect(result.section).toBe('new-section');
...
`;
        ...
        await expect(performSectionEdit(manager, testDocPath, 'tasks', 'Content', 'append', undefined // No title provided
        )).rejects.toThrow();
    });
...
`;
        await fs.writeFile(testDocAbsolutePath, testContent, 'utf8');
        // First append
        await performSectionEdit(manager, testDocPath, 'tasks', 'Section A content', 'append', 'Section A');
        // Second append (should also go before tasks)
        await performSectionEdit(manager, testDocPath, 'tasks', 'Section B content', 'append', 'Section B');
...
`;
        await fs.writeFile(testDocAbsolutePath, testContent, 'utf8');
        // Append section
        const result = await performSectionEdit(manager, testDocPath, 'tasks', 'New content', 'append', 'New Section');
...
//# sourceMappingURL=section-operations.append.test.js.map