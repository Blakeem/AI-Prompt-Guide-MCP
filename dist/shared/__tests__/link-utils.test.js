...
import { parseLink, resolveLink, validateLink, linkExists, resolveLinkWithContext } from '../link-utils.js';
...
describe('resolveLink Function', () => {
    describe('Input Validation', () => {
        test('should throw error for empty current document path', () => {
            const link = { type: 'within-doc', raw: '@#section' };
    ...
    describe('External Links', () => {
        test('should return external links as-is', () => {
            const link = { type: 'external', raw: 'https://example.com' };
            const result = resolveLink(link, '/current/doc.md');
            expect(result).toBe('https://example.com');
        });
        test('should return complex external links as-is', () => {
            const link = { type: 'external', raw: 'mailto:test@example.com?subject=Hello' };
            const result = resolveLink(link, '/current/doc.md');
    ...
    describe('Within-Document Links', () => {
        test('should resolve section links to current document', () => {
            const link = { type: 'within-doc', raw: '@#overview', section: 'overview' };
        ...
        test('should resolve document-only within-doc links', () => {
            const link = { type: 'within-doc', raw: '@#' };
            const result = resolveLink(link, '/api/users.md');
        ...
        test('should normalize current document path', () => {
            const link = { type: 'within-doc', raw: '@#section', section: 'section' };
    ...
    describe('Cross-Document Links', () => {
        ...
        test('should throw error for empty document path', () => {
            const link = { type: 'cross-doc', raw: '@', document: '' };
            expect(() => resolveLink(link, '/current/doc.md')).toThrow('Cross-document link missing document path');
        });
    });
    ...
    describe('Error Cases', () => {
        test('should throw error for unknown link type', () => {
            const link = { type: 'unknown', raw: '@test' };
...
describe('validateLink Function', () => {
    ...
    describe('Input Validation', () => {
        test('should reject empty link path', async () => {
            const result = await validateLink('', mockDocumentManager);
            expect(result).toEqual({
                valid: false,
    ...
    describe('External Links', () => {
        test('should validate HTTP URLs as valid', async () => {
            const result = await validateLink('https://example.com', mockDocumentManager);
            expect(result).toEqual({
                valid: true
            });
        });
        test('should validate www URLs as valid', async () => {
            const result = await validateLink('www.example.com', mockDocumentManager);
            expect(result).toEqual({
                valid: true
            });
        });
        test('should validate complex URLs as valid', async () => {
            const result = await validateLink('https://api.example.com/v1/docs?section=auth', mockDocumentManager);
            expect(result).toEqual({
                valid: true
            });
        });
    ...
    describe('Document Validation', () => {
        test('should validate existing document without section', async () => {
            ...
            const result = await validateLink('/api/users.md', mockDocumentManager);
            expect(result).toEqual({
                valid: true,
                documentExists: true
            });
        ...
        test('should reject non-existent document', async () => {
            ...
            const result = await validateLink('/api/missing.md', mockDocumentManager);
            expect(result).toEqual({
                valid: false,
    ...
    describe('Section Validation', () => {
        test('should validate existing section', async () => {
            mockDocumentManager.getDocument.mockResolvedValue(sampleDocument);
            const result = await validateLink('/api/users.md#authentication', mockDocumentManager);
            expect(result).toEqual({
                valid: true,
                documentExists: true,
                sectionExists: true
        ...
        test('should reject non-existent section with suggestions', async () => {
            mockDocumentManager.getDocument.mockResolvedValue(sampleDocument);
            const result = await validateLink('/api/users.md#missing-section', mockDocumentManager);
        ...
        test('should handle documents with no sections', async () => {
            ...
            const result = await validateLink('/api/users.md#any-section', mockDocumentManager);
            expect(result).toEqual({
                valid: false,
                documentExists: true,
                sectionExists: false,
    ...
    describe('Complex Cases', () => {
        test('should handle multiple hash symbols correctly', async () => {
            mockDocumentManager.getDocument.mockResolvedValue(sampleDocument);
            const result = await validateLink('/api/users.md#section#subsection', mockDocumentManager);
            expect(result.valid).toBe(false);
            expect(result.documentExists).toBe(true);
            expect(result.sectionExists).toBe(false);
        ...
        test('should handle document lookup errors during section validation', async () => {
            const docWithError = {
                ...sampleDocument,
                headings: null // Simulate corrupt data
            };
...
describe('linkExists Function', () => {
    ...
    test('should return true for valid links', async () => {
        ...
        const result = await linkExists('/api/users.md#authentication', mockDocumentManager);
        expect(result).toBe(true);
    });
    test('should return false for invalid links', async () => {
        mockDocumentManager.getDocument.mockResolvedValue(null);
        const result = await linkExists('/api/missing.md', mockDocumentManager);
    ...
    test('should return false when validation throws error', async () => {
        ...
        const result = await linkExists('/api/users.md', mockDocumentManager);
        expect(result).toBe(false);
    });
    test('should handle external links correctly', async () => {
        const result = await linkExists('https://example.com', mockDocumentManager);
        expect(result).toBe(true);
    });
});
describe('resolveLinkWithContext Function', () => {
    ...
    test('should resolve valid link with context', async () => {
        mockDocumentManager.getDocument.mockResolvedValue(sampleDocument);
        const result = await resolveLinkWithContext('@/api/users.md#authentication', '/current/doc.md', mockDocumentManager, { includeSuggestions: true });
    ...
    test('should handle link resolution errors', async () => {
        const result = await resolveLinkWithContext('@', '/current/doc.md', mockDocumentManager);
    ...
    test('should handle invalid links with validation errors', async () => {
        mockDocumentManager.getDocument.mockResolvedValue(null);
        const result = await resolveLinkWithContext('@/api/missing.md', '/current/doc.md', mockDocumentManager);
        expect(result.parsed.type).toBe('cross-doc');
    ...
    test('should resolve external links correctly', async () => {
        const result = await resolveLinkWithContext('https://example.com', '/current/doc.md', mockDocumentManager);
    ...
    test('should handle within-document links', async () => {
        ...
        const result = await resolveLinkWithContext('@#authentication', '/current/doc.md', mockDocumentManager);
    ...
    test('should include context when requested and validation passes', async () => {
        mockDocumentManager.getDocument.mockResolvedValue(sampleDocument);
        const result = await resolveLinkWithContext('@/api/users.md', '/current/doc.md', mockDocumentManager, { includeSuggestions: true });
        expect(result.validation.valid).toBe(true);
...
//# sourceMappingURL=link-utils.test.js.map