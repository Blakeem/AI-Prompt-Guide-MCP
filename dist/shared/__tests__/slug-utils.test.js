...
import { generateHierarchicalSlug, splitSlugPath, joinSlugPath, getSlugDepth, getParentSlug, getSlugLeaf, createHierarchicalSlug, isSlugAncestor, isDirectChild, getDirectChildren, getAllDescendants, getRelativeSlugPath, validateSlugPath } from '../slug-utils.js';
// Mock the titleToSlug function
vi.mock('../../slug.js', () => ({
    titleToSlug: vi.fn((title) => title.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, ''))
}));
...
describe('generateHierarchicalSlug Function', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        mockTitleToSlug.mockImplementation((title) => title.toLowerCase().replace(/\s+/g, '-').replace(/[^\w-]/g, ''));
    });
    describe('Input Validation', () => {
        ...
        test('should throw error for non-string child title', () => {
            expect(() => generateHierarchicalSlug('parent', null)).toThrow('Child title is required for hierarchical slug generation');
        });
    });
    describe('Basic Slug Generation', () => {
        ...
        test('should handle null parent slug', () => {
            ...
            const result = generateHierarchicalSlug(null, 'Overview');
...
describe('splitSlugPath Function', () => {
    describe('Input Validation', () => {
        test('should handle non-string input', () => {
            expect(splitSlugPath(null)).toEqual([]);
            expect(splitSlugPath(undefined)).toEqual([]);
            expect(splitSlugPath(123)).toEqual([]);
        });
...
describe('joinSlugPath Function', () => {
    describe('Input Validation', () => {
        test('should handle non-array input', () => {
            expect(joinSlugPath(null)).toBe('');
            expect(joinSlugPath(undefined)).toBe('');
            expect(joinSlugPath('string')).toBe('');
        });
    ...
    describe('Component Filtering', () => {
        ...
        test('should filter non-string components', () => {
            expect(joinSlugPath(['api', null, 'users', undefined])).toBe('api/users');
        });
...
describe('getDirectChildren Function', () => {
    ...
    test('should handle non-array input', () => {
        const result = getDirectChildren('api', null);
        expect(result).toEqual([]);
    });
});
describe('getAllDescendants Function', () => {
    ...
    test('should handle non-array input', () => {
        const result = getAllDescendants('api', null);
        expect(result).toEqual([]);
    });
});
...
describe('validateSlugPath Function', () => {
    ...
    test('should reject non-string input', () => {
        const result = validateSlugPath(null);
        expect(result).toEqual({
            success: false,
...
//# sourceMappingURL=slug-utils.test.js.map