...
import { validateSingleLink, validateDocumentLinks, validateSystemLinks, autoFixLinks } from '../link-validation.js';
...
vi.mock('../utilities.js', () => ({
    pathToNamespace: vi.fn((path) => path.split('/').slice(0, -1).join('.').replace(/^\./, '') || 'root')
}));
import { parseLink, validateLink } from '../link-utils.js';
import { pathToNamespace } from '../utilities.js';
const mockParseLink = parseLink;
const mockValidateLink = validateLink;
const mockPathToNamespace = pathToNamespace;
...
const createSampleDocument = (path = '/api/users.md') => ({
    metadata: {
        path,
...
describe('validateSingleLink Function', () => {
    ...
    beforeEach(async () => {
        ...
        mockPathToNamespace.mockImplementation((path) => path.split('/').slice(0, -1).join('.').replace(/^\./, '') || 'root');
    });
    afterEach(async () => {
    ...
    describe('External Links', () => {
        test('should validate external links as valid', async () => {
            ...
            const result = await validateSingleLink('https://example.com', '/current/doc.md', mockDocumentManager);
        ...
        test('should handle complex external URLs', async () => {
            ...
            const result = await validateSingleLink('https://api.example.com/v1/docs?section=auth#overview', '/current/doc.md', mockDocumentManager);
    ...
    describe('Cross-Document Links', () => {
        test('should validate valid cross-document links', async () => {
            ...
            const result = await validateSingleLink('@/api/users.md#get-user', '/current/doc.md', mockDocumentManager);
            expect(result).toEqual({
        ...
        test('should handle invalid cross-document links with suggestions', async () => {
            ...
            const result = await validateSingleLink('@/api/missing.md', '/current/doc.md', mockDocumentManager);
    ...
    describe('Within-Document Links', () => {
        test('should validate within-document links', async () => {
            ...
            const result = await validateSingleLink('@#authentication', '/api/users.md', mockDocumentManager);
            expect(result).toEqual({
        ...
        test('should handle invalid within-document sections', async () => {
            ...
            const result = await validateSingleLink('@#missing-section', '/api/users.md', mockDocumentManager);
            expect(result.is_valid).toBe(false);
    ...
    describe('Malformed Links', () => {
        test('should handle parsing errors gracefully', async () => {
            ...
            const result = await validateSingleLink('malformed@link', '/current/doc.md', mockDocumentManager);
            expect(result.is_valid).toBe(false);
        ...
        test('should handle validation errors', async () => {
            ...
            const result = await validateSingleLink('@/api/users.md', '/current/doc.md', mockDocumentManager);
            expect(result.is_valid).toBe(false);
...
describe('validateDocumentLinks Function', () => {
    ...
    test('should generate comprehensive link report for document', async () => {
        ...
        const result = await validateDocumentLinks('/api/users.md', mockDocumentManager);
    ...
    test('should handle document not found', async () => {
        ...
        await expect(validateDocumentLinks('/missing/doc.md', mockDocumentManager)).rejects.toThrow('Document not found: /missing/doc.md');
    });
...
describe('validateSystemLinks Function', () => {
    ...
    test('should validate links across multiple documents', async () => {
        ...
        const result = await validateSystemLinks(mockDocumentManager);
    ...
    test('should filter documents by path', async () => {
        ...
        const result = await validateSystemLinks(mockDocumentManager, '/api');
    ...
    test('should handle document list retrieval errors', async () => {
        ...
        await expect(validateSystemLinks(mockDocumentManager)).rejects.toThrow('Failed to get document list: Database error');
    });
    test('should handle individual document validation errors gracefully', async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith('Failed to validate links in /api/broken.md:', expect.any(Error));
...
describe('autoFixLinks Function', () => {
    ...
    test('should identify fixable link issues in dry run mode', async () => {
        ...
        const result = await autoFixLinks('/api/users.md', mockDocumentManager, true // dry run
        );
    ...
    test('should handle document not found', async () => {
        mockDocumentManager.getDocument.mockResolvedValue(null);
        await expect(autoFixLinks('/missing.md', mockDocumentManager)).rejects.toThrow('Document not found: /missing.md');
    });
    test('should handle section reading errors gracefully', async () => {
        ...
        const result = await autoFixLinks('/api/users.md', mockDocumentManager);
    ...
    test('should not apply fixes in dry run mode', async () => {
        ...
        const result = await autoFixLinks('/api/users.md', mockDocumentManager, true);
    ...
    test('should prepare fixes for non-dry-run mode', async () => {
        ...
        const result = await autoFixLinks('/api/users.md', mockDocumentManager, false);
        expect(result.fixes_applied).toBe(0); // Not implemented yet
    });
});
//# sourceMappingURL=link-validation.test.js.map