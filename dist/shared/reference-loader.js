...
export class ReferenceLoader {
     ...
     * ```
     */
    async loadReferences(refs, manager, maxDepth = 3, currentDepth = 0, visitedPaths, totalNodesLoaded, startTime) {
        // Input validation
        ...
        if (Date.now() - operationStart > this.DEFAULT_TIMEOUT_MS) {
            throw new Error(`Reference loading operation exceeded timeout of ${this.DEFAULT_TIMEOUT_MS}ms. ` +
        ...
        const pathsToTrack = visitedPaths ?? new Set();
        const results = [];
        for (const ref of refs) {
            try {
                const content = await this.loadSingleReference(ref, manager, maxDepth, currentDepth, pathsToTrack, nodeTracker, operationStart);
                if (content != null) {
     ...
     */
    async loadSingleReference(ref, manager, maxDepth, currentDepth, visitedPaths, nodeTracker, startTime) {
        ...
        const normalizedNestedRefs = this.extractor.normalizeReferences(nestedRefs, ref.documentPath);
        // Filter out self-references and already visited paths (with cycle detection logging)
        const filteredNestedRefs = normalizedNestedRefs.filter(nestedRef => {
        ...
        const children = await this.loadReferences(filteredNestedRefs, manager, maxDepth, currentDepth + 1, visitedPaths, nodeTracker, startTime);
     ...
     * ```
     */
    async loadReferencesFromContent(content, contextPath, manager, maxDepth = 3) {
