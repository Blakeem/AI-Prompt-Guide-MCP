...
vi.mock('../../utils/logger.js', () => ({
  getGlobalLogger: (): Record<string, () => void> => ({
    debug: vi.fn(),
    info: vi.fn(),
    warn: vi.fn(),
    error: vi.fn()
...
// Test utilities
const createMockDocument = (overrides: Partial<CachedDocument> = {}): CachedDocument => ({
  metadata: {
...
const createMockDocumentManager = (
  documents: Array<{ path: string; document: CachedDocument }> = [],
...
describe('Enhanced Document Analysis Integration', () => {
  describe('Related Document Discovery with Enhanced Features', () => {
    ...
    it('should gracefully fall back when fingerprint optimization fails', async () => {
      ...
      (manager.listDocumentFingerprints as any).mockRejectedValue(new Error('Fingerprint system unavailable'));

      const result = await analyzeDocumentSuggestions(
        manager,
        'api/auth',
        'Authentication Guide',
        'Guide for implementing authentication'
      );
  ...
  describe('Structured Broken Reference Detection', () => {
    it('should detect and classify missing documents', async () => {
      ...
      const result = await analyzeDocumentSuggestions(
        ...
        'test',
        'Reference Test',
        'Document with references: @/missing/doc.md and @/another/missing.md#section'
      );
    ...
    it('should detect missing sections in existing documents', async () => {
      ...
      const result = await analyzeDocumentSuggestions(
        ...
        'Reference to existing doc with missing section: @/api/auth.md#nonexistent-section'
      );
    ...
    it('should detect malformed references', async () => {
      ...
      const result = await analyzeDocumentSuggestions(
        ...
        'Document with malformed references: @[invalid-format] and @incomplete'
      );
    ...
    it('should handle relative references correctly', async () => {
      ...
      const result = await analyzeDocumentSuggestions(
        ...
        'Document with relative references: @relative-doc and @another-relative.md'
      );
    ...
    it('should deduplicate broken references', async () => {
      ...
      const result = await analyzeDocumentSuggestions(
        ...
        'Document with duplicate references: @/missing/doc.md and @/missing/doc.md again'
      );
  ...
  describe('Error Handling and Graceful Degradation', () => {
    it('should handle analysis failures with partial results', async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (failingManager.listDocuments as any).mockRejectedValue(new Error('Database connection failed'));
    ...
    it('should validate input parameters', async () => {
      ...
      await expect(analyzeDocumentSuggestions(
        null as unknown as DocumentManager,
        'test',
        'Test',
        'Test'
    ...
    it('should handle document read failures gracefully', async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (manager.getDocumentContent as any).mockRejectedValue(new Error('File read error'));

      const result = await analyzeDocumentSuggestions(
        manager,
        'test',
        'Test Document',
        'Test content with failing keywords'
      );
  ...
  describe('Performance and Scalability', () => {
    it('should handle large document sets efficiently', async () => {
      ...
      const result = await analyzeDocumentSuggestions(
        ...
        'Document for testing performance with many potential matches'
      );
    ...
    it('should optimize with fingerprint-based two-stage filtering', async () => {
      ...
      const result = await analyzeDocumentSuggestions(
        ...
        'namespace-0',
        'Optimization Test',
        'Document with common shared keywords for testing optimization'
      );
      const duration = Date.now() - startTime;
