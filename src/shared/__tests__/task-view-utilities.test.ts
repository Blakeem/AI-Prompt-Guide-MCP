...
→ @/path/to/doc.md
  ...
  describe('formatTaskResponse', () => {
    ...
    it('should omit undefined optional fields', () => {
      const minimalTask: TaskViewData = {
        slug: 'minimal',
        title: 'Minimal Task',
        content: 'content',
        status: 'pending'
...
→ https://example.com
      ...
      vi.doMock('../../config.js', () => ({
        loadConfig: (): { referenceExtractionDepth: number } => ({ referenceExtractionDepth: 1 })
      }));

      vi.doMock('../reference-extractor.js', () => ({
        ReferenceExtractor: class {
          extractReferences(): string[] { return []; }
          normalizeReferences(): string[] { return []; }
        }
      }));

      vi.doMock('../reference-loader.js', () => ({
        ReferenceLoader: class {
          async loadReferences(): Promise<string[]> { return []; }
        }
      }));
