...
export async function editTaskOperation(
  ...
  try {
    ...
    throw new AddressingError(
      ...
      'TASK_EDIT_FAILED',
      { document: documentPath, task: taskSlug }
    );
  }
}
 ...
 * @throws {DocumentNotFoundError} When document doesn't exist
 */
export async function ensureTasksSectionOperation(manager: DocumentManager, docPath: string): Promise<void> {
 ...
 */
function findNextTask(tasks: TaskData[]): {
  slug: string;
  title: string;
  link?: string;
} | undefined {
