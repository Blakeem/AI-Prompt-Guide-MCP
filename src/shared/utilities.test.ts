...
// Mock imports
vi.mock('../slug.js', () => ({
  titleToSlug: vi.fn((title: string) => title.toLowerCase().replace(/\s+/g, '-'))
}));
...
describe('performSectionEdit Utility Function', () => {
  ...
  describe('Document Validation', () => {
    test('should throw error when document does not exist', async () => {
      ...
      ).rejects.toThrow('Document not found: /non-existent.md');
    });
  ...
  describe('Edit Operations', () => {
    ...
    test('should handle replace operation successfully', async () => {
      ...
      // Assert
      expect(mockDocumentManager.updateSection).toHaveBeenCalledWith(
        '/test-doc.md',
        'overview',
        content,
        {
    ...
    test('should handle append operation with existing content', async () => {
      ...
      // Assert
      expect(mockDocumentManager.getSectionContent).toHaveBeenCalledWith(
        '/test-doc.md',
        'overview'
      );
    ...
    test('should handle append operation with empty existing content', async () => {
      ...
      expect(mockDocumentManager.updateSection).toHaveBeenCalledWith(
        '/test-doc.md',
        'overview',
        newContent, // Should use new content directly
        {
          updateToc: true,
          validateLinks: true
        }
  ...
  describe('Creation Operations', () => {
    ...
    test('should handle insert_before operation successfully', async () => {
      ...
      // Assert
      expect(mockDocumentManager.insertSection).toHaveBeenCalledWith(
        '/test-doc.md',
        'features',
        'insert_before' as InsertMode,
        undefined, // Auto-calculate depth
        title,
        content,
        { updateToc: true }
      );
    ...
    test('should throw error for creation operation with empty title', async () => {
      ...
      ).rejects.toThrow('Title is required for creation operation: insert_after');
