...
export class FingerprintIndex {
  ...
  private readonly fingerprints = new Map<string, FingerprintEntry>();
   ...
   */
  private async findAllMarkdownFiles(dir: string, files: string[] = []): Promise<string[]> {
   ...
   */
  private async indexDocument(absolutePath: string): Promise<void> {
   ...
   */
  getStats(): { documents: number; keywords: number; avgKeywordsPerDoc: number } {
    return {
      documents: this.fingerprints.size,
      keywords: this.keywordIndex.size,
