...
const createMockSmartSuggestions = (overrides: Partial<SmartSuggestions> = {}): SmartSuggestions => ({
  related_documents: [
    {
      path: '/api/auth.md',
...
const createMockNamespacePatterns = (): { common_sections: string[]; frequent_links: string[]; typical_tasks: string[] } => ({
...
describe('Suggestion Generator (Stage 2.5)', () => {
  ...
  describe('processSuggestions', () => {
    it('should return smart suggestions result with related documents', async () => {
      ...
      const smartResult = result as Extract<SuggestionsResult, { stage: 'smart_suggestions' }>;
    ...
    it('should handle empty suggestions gracefully', async () => {
      ...
      const result = await processSuggestions(
        'new-namespace',
        'First Document',
        'Starting a new documentation namespace'
      );
    ...
    it('should run suggestions and namespace analysis in parallel', async () => {
      ...
      expect(mockAnalyzeDocumentSuggestions).toHaveBeenCalledWith(
        mockManager,
        'api',
        'Test',
        'Test overview'
  ...
  describe('error handling', () => {
    it('should return error result when document manager fails', async () => {
      ...
      const errorResult = result as Extract<SuggestionsResult, { stage: 'error_fallback' }>;
  ...
  describe('integration scenarios', () => {
    ...
    it('should handle namespace with no existing documents', async () => {
      ...
      const result = await processSuggestions(
        'brand-new-namespace',
        'First Document Ever',
        'Starting completely fresh documentation'
      );
    ...
    it('should handle complex broken references scenario', async () => {
      ...
      const result = await processSuggestions(
        'api',
        'Migration Guide',
        'Guide for migrating from old API to new. See @/api/v1/deprecated.md and @missing-entirely.md'
      );
    ...
    it('should validate comprehensive BrokenReference structure integration', async () => {
      ...
      const result = await processSuggestions(
        ...
        'Test all reference types: @/missing/document.md @/existing/document.md#missing-section @malformed-ref'
      );
