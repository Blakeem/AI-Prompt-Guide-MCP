...
`;
  ...
  it('should load content from documents referenced with @/path/doc.md format', async () => {
    ...
    if (result.linked_context != null) {
      ...
      const toolTestingRef = result.linked_context.find(
        ctx => ctx.document_path === '/docs/tasks/tool-testing-results.md'
      );
  ...
  it('should load specific section when reference includes #section', async () => {
    ...
    if (result.linked_context != null) {
      ...
      const testDocRef = result.linked_context.find(
        ctx => ctx.document_path === '/docs/specs/test-doc.md' && ctx.section_slug === 'setup'
      );
