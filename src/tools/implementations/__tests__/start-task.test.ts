...
Implement the feature.`;
      ...
      vi.spyOn(manager, 'getSectionContent').mockImplementation(async (_path, slug) => {
        if (slug === 'design-architecture') return firstTaskContent;
        if (slug === 'implement-feature') return currentTaskContent;
        return null;
      });
      ...
      vi.doMock('../../../prompts/workflow-prompts.js', () => ({
        getWorkflowPrompt: vi.fn((name: string) => {
          ...
          if (name === 'simplicity-gate') return mockTaskWorkflow;
          return undefined;
        }),
...
Second task.`;
      ...
      vi.spyOn(manager, 'getSectionContent').mockImplementation(async (_path, slug) => {
        ...
        if (slug === 'second-task') return currentTaskContent;
        return null;
      });
...
Implement the REST API endpoints.`;
      ...
      vi.spyOn(manager, 'getDocument').mockResolvedValue(mockDocument);
      vi.spyOn(manager, 'getSectionContent').mockImplementation(async (_path, slug) => {
        if (slug === 'design-system') return firstTaskContent;
        if (slug === 'implement-api') return currentTaskContent;
        return null;
      });
